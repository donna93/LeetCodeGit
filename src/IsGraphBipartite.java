...
public class IsGraphBipartite {
    public boolean isBipartite(int[][] graph) {
        ...
        for (int i = 0; i < colors.length; i++){
            if (colors[i] == -1 && !isBipartiteGraph(i, 0, graph, colors)){
                return false;
            }
        }
        return true;
    }
    public boolean isBipartiteGraph(int curNode, int curColor, int[][] graph, int[] colors){
        if (colors[curNode] != -1){
            return colors[curNode] == curColor;
        }
        colors[curNode] = curColor;
        for (int nextNode : graph[curNode]){
            if (!isBipartiteGraph(nextNode, 1 - curColor, graph, colors)){
                return false;
            }
        }
        return true;
